...
系统的贴靠布局只有 Windows 11 有。本库另有一套自己的，从布局表到窗口摆放全是自己的代码，
四个平台一致。两者由 `TitleBar.SnapLayoutMode` 二选一。

**为什么只能二选一。**系统那套要求最大化钮在 `WM_NCHITTEST` 上返回 `HTMAXBUTTON`，
而标成非客户区之后，那块像素的指针事件不再送到 Avalonia——自绘面板的悬停就永远触发不了。
所以 `Builtin` 模式下最大化钮必须标回 `Client`。运行时改模式要重标一遍，
否则切过去之后新面板弹不出来，而且不报错。

**分区几何：取整取的是边界，不是尺寸。**这条是整套东西里最容易写错的一处：

```csharp
// 对：两条边界各自从比例取整，相邻分区共用同一个值
var left  = origin + (int)Math.Round(extent * zone.X);
var right = origin + (int)Math.Round(extent * zone.Right);

// 错：分别取整宽度。1919 三等分，639.67 各自取整成 640，合计 1920——
// 最右边那栏溢出屏幕一像素；取 639 则在右边留一条 2 像素的缝。
var width = (int)Math.Round(extent * zone.Width);
```

两种错法都不报错、不崩，在开发机上肉眼几乎看不出来，到 4K 面板上并排三个窗口才发现边缘对不齐。

**门槛按逻辑像素判。**「屏幕够不够宽给三栏」要除以 `Screen.Scaling` 再比。
按物理像素判的话，13 寸 2560×1600、150% 缩放的笔记本会被当成 2560 宽——
而那台机器上三等分每栏只有 569 逻辑像素。这个错在开发机（多半接 100% 缩放的外接屏）上
永远复现不出来。

**Position 是外框，Width/Height 是客户区。**用系统装饰时两者差一圈边框，
不减掉的话贴靠后的窗口会比分区大出一圈，右边和下边压在隔壁分区上。
扩展客户区（自绘标题栏）时这一圈是 0。

**还原点只记第一次。**已经贴靠的窗口再贴到别处，还原点保持第一次贴靠之前的那个。
每贴一次就更新的话，来回换几次布局之后就再也回不到原始尺寸——
而操作员以为「取消贴靠」总能回到他一开始那个窗口。

**能摆的只有本进程自己的窗口。**Windows 的贴靠助手会在剩下的分区里列出别的应用的窗口，
那需要系统级权限，本库做不到，也不该假装做得到。

**无头环境测不到的，抽成纯函数。**缩放恒为 1、没有系统装饰、屏幕信息永远拿得到——
DPI 换算、边框补偿、单窗口平台的降级这三条在无头测试里全无约束力。
它们各自抽成了静态纯函数（`WindowSnap.ClientSizeFor` / `FramePadding`、
`TitleBar.ResolveSnapMode`），在测试里穷举。留在实例方法里就只能靠读代码判断对不对。
